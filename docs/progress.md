...
| --- | --- | --- | --- | --- |
| Text validation/fix | [~] | Implemented | Needs real-mask test | P0 crash/freeze hotfix is implemented: async command failures are contained, Text Creator preview is debounced and list-replaced instead of cell-mutated, quick captions use centered/fitted 44-column payloads with black/off blank padding, two-word captions such as `VIBE CHECK` render on two centered rows, quick-action defaults use paced Fast write-only with Flash/Blink mode 2 at speed 100, and text uploads now serialize with a pre-upload text-mode reset to reduce shifted repeated sends. Validate on physical iOS first, then Android. |
...
| Text | [~] | Deterministic ASCII glyph rasterizer, text upload payload/framing, encrypted `DATS`/`DATCP`/`MODE`/speed commands, documented text modes, broader ACK parsing, simulator transport, MAUI composer, debounced/list-replaced LED preview, truncated diagnostics, transport state events, ACK capability reporting, ACK-required upload mode, write-only compatibility upload mode, paced quick-caption send option, serialized upload transactions, pre-upload text-mode reset, quick-caption 44-column centered/fitted/two-line payloads, black/off quick-caption blank columns, tests, iOS upload transport, Android BLE upload transport, and compile validation. | Improve text creation/editing flow with smooth preview feedback, visible transport readiness, explicit ACK/write-only mode, send progress, no crash on text tap, no typing freeze, and actionable diagnostics before claiming real hardware parity. | Validate ACK-required and write-only text upload on physical iOS first, then Android; confirm `VIBE CHECK`, `LOL`, `DROP`, and a long RAVE caption blink/flash centered/fitted on the real mask without right-side artifacts or intermittent right-shift on repeated sends. |
