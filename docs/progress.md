...
| Built-in Scanner + Archive | [~] | Implemented | Needs real-mask test | Built-ins tab can scan/send `IMAG` and `ANIM` IDs, save metadata-only archive records, favorite tested IDs, and surface favorites in React/RAVE; validate useful IDs and persistence on a real mask/iPhone. |
...
| Reaction Deck | [~] | React page/view model groups short offline reactions by catalog category, pins BLACKOUT plus Random Reaction, and can show favorite archived built-ins when saved records exist. | Give the wearer a one-tap deck for social/meme/welfare reactions and reusable stock faces. | Validate short caption sends, favorite built-in sends, and write-only/ACK status on a real mask. |
| RAVE MVP | [~] | RAVE page/view model provides large manual DnB/festival buttons, command-only built-in fallbacks, favorite archived built-ins under secondary controls, BLACKOUT, brightness cap, Festival Lock, connect action, offline/manual status, and shared intent dispatch. | Make the mask useful in a loud/dark festival setting without internet, AI, microphone, or automation. | Validate physical text timing, command fallbacks, favorite built-ins, blackout, brightness cap, and reconnect in iOS real-mask testing. |
| Built-in Scanner + Archive | [~] | Built-ins page/view model sends static image and animation IDs through command transport, shows decimal/hex IDs, safe range notes, metadata fields, status/favorite controls, local JSON archive persistence, favorites/tested list, last tested/send status, transport readiness, and BLACKOUT. | Let the wearer discover, name, classify, and reuse useful stock-firmware visuals on the real mask before the festival. | Test `IMAG`/`ANIM` IDs on physical iPhone/mask, save useful records, reopen the app, and confirm archive persistence. |
...
- [x] Add built-in archive metadata models and MaskPack manifest models/parser for future app-owned custom content. Built-in archive records are metadata only and do not extract stock mask frames.
