...
| Control Room | [~] | Home page/view model now shows control/text transport status, recovery hints, foreground-only auto-connect state, known-mask memory, brightness, BLACKOUT, restore, random reaction, favorite reactions, recent reactions, favorite text presets, Text Composer entry, and persisted quick-caption settings through shared quick-action intents. | Make the first screen useful for live mask operation instead of showing a migration roadmap. | Validate open-connect, remembered-mask auto-connect, blackout, brightness cap, per-preset text style, Flash/Blink quick captions, favorite preset sends, and short reactions on a real mask. |
| Reaction Deck | [~] | React page/view model groups local text presets by Czech Basic, Czech Meme, Czech Political/Satire, Czech RAVE, and Custom, keeps hardcoded quick actions as fallback groups, pins BLACKOUT plus Random Reaction, links to Text Composer, and can show favorite archived built-ins as command-only one-tap faces when saved records exist. | Give the wearer a one-tap deck for social/meme/welfare reactions, Czech captions, editable satire text, and reusable stock faces. | Validate short preset sends, mask-safe transliteration, favorite built-in sends, and write-only/ACK status on a real mask. |
| RAVE MVP | [~] | RAVE page/view model provides RAVE favorite text presets, Czech RAVE presets, large manual DnB/festival buttons, command-only built-in fallbacks, favorite archived built-ins, BLACKOUT, brightness cap, sticky Text/Connect/BLACKOUT actions, concise live statuses, global foreground text summary, and shared intent dispatch. Festival Lock no longer gates normal controls. | Make the mask useful in a loud/dark festival setting without internet, AI, microphone, or automation. | Validate physical text timing, Czech RAVE preset sends, command fallbacks, favorite built-ins, blackout, brightness cap, per-preset foreground colors, and reconnect/auto-connect in iOS real-mask testing. |
...
- [x] Physical-feedback follow-up for checklist item 8/11: quick actions now use paced Fast write-only sends, and Text Creator preview refresh is debounced so typing does not synchronously rebuild the LED grid; validated with `dotnet test tests\MaskApp.Core.Tests\MaskApp.Core.Tests.csproj` (116 tests), Roslyn diagnostics (0 warnings, 0 errors), iOS build, and Android build.
