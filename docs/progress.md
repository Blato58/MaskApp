...
| Image Studio and DIY slots | [~] | Implemented | Needs real-mask test | Face Studio now supports 36x12 static face drawing, photo/camera import, six generated smiley emotions, local JSON persistence, and `DATS`/frames/`DATCP`/`PLAY` upload wiring. Validate real-mask image appearance, slot overwrite behavior, `CHEC`, `DELE`, and repeated `PLAY` before relying on slot sequencing. |
...
| Image | [~] | Face Studio supports 36x12 static faces, six built-in smiley emotions, touch drawing, clear/mirror/color tools, photo/camera import into the LED grid, local JSON face persistence, Gallery/Pages projection, and DIY slot upload/play through iOS and Android BLE adapters. Custom animation and MaskPack playback remain future/Labs. | Let the wearer make or import a simple expression, preview the exact mask grid, save it, and send it as a DIY face without leaving the Library/Page workflow. | Physically validate real-mask `DATS`/frame/`DATCP` upload, slot overwrite behavior, `PLAY`, `CHEC`, and `DELE`; inspect rendered Face Studio ergonomics on iPhone. |
...
- [~] Port DIY/image frame splitting and acknowledgement parsing from `docs/stock-mask-protocol.md` with tests. Static 36x12 face upload/play is implemented; `CHEC`, `DELE`, fast sequencing, and physical visual confirmation remain open.
