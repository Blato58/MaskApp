...
- [x] Add Face Studio with six generated smiley faces, 36x12 pixel editing, photo/camera import, local face persistence, and DIY slot upload/play wiring.
...
| Image Studio and DIY slots | [~] | Implemented | Needs real-mask test | Face Studio now supports 36x12 static face drawing, photo/camera import, six generated smiley emotions, local JSON persistence, and Java-compatible column-major RGB-only `DATS`/frames/`DATCP`/`PLAY` upload wiring. Earlier real-mask feedback showed split white/yellow output with black patches from the old prefixed/row-major encoder. Validate corrected real-mask image appearance, slot overwrite behavior, `CHEC`, `DELE`, and repeated `PLAY` before relying on slot sequencing. |
...
| Image | [~] | Face Studio supports 36x12 static faces, six built-in smiley emotions, touch drawing, clear/mirror/color tools, photo/camera import into the LED grid, local JSON face persistence, Gallery/Pages projection, and Java-compatible RGB-only DIY slot upload/play through iOS and Android BLE adapters. Custom animation and MaskPack playback remain future/Labs. | Let the wearer make or import a simple expression, preview the exact mask grid, save it, and send it as a DIY face without leaving the Library/Page workflow. | Physically validate corrected real-mask `DATS`/frame/`DATCP` upload, slot overwrite behavior, `PLAY`, `CHEC`, and `DELE`; inspect rendered Face Studio ergonomics on iPhone. |
...
| Storage | [~] | [~] | Built-in archive metadata, text presets, gallery layout, quick-caption settings, known-mask settings, and Face Studio patterns use simple versioned JSON in app data. GreenDAO replacement for broader migrated data is not selected yet. |
| Camera/media picker | [~] | [~] | Face Studio uses .NET MAUI MediaPicker for photo import/camera capture, with native iOS/Android pixel decoders and Android media/camera manifest entries. Physical device picker/camera UX validation remains open. |
...
- [x] Add built-in archive metadata models, fast favorite/working Faces deck behavior, Face Studio app-owned static faces, and MaskPack manifest models/parser for future app-owned custom content. Built-in archive records are metadata only and do not extract stock mask frames; Custom Animations, MaskPacks, and advanced Mood/Expression categories remain future/Labs until upload/playback validation.
...
- [~] Port DIY/image frame splitting and acknowledgement parsing from `docs/stock-mask-protocol.md` with tests. Static 36x12 face upload/play now uses Java-compatible column-major RGB-only payloads; previous prefixed/row-major payloads failed real-mask visual feedback with split colors and black patches. `CHEC`, `DELE`, fast sequencing, and corrected physical visual confirmation remain open.
...
- [x] Library/Pages compact-header and icon preview fix validated with `dotnet test tests\MaskApp.Core.Tests\MaskApp.Core.Tests.csproj --no-restore` (202 tests), `dotnet build src\MaskApp.App\MaskApp.App.csproj -f net10.0-ios --no-restore` (0 warnings, 0 errors), `dotnet build src\MaskApp.App\MaskApp.App.csproj -f net10.0-android --no-restore` (0 warnings, 0 errors), and `git diff --check`. Physical/rendered validation remains required to confirm icons display, collapsed headers feel right, and tile gaps are sufficient on device.
- [x] Custom faces and smileys slice validated with `dotnet test tests\MaskApp.Core.Tests\MaskApp.Core.Tests.csproj` (216 tests), `dotnet build src\MaskApp.App\MaskApp.App.csproj -f net10.0-ios` (0 warnings, 0 errors), `dotnet build src\MaskApp.App\MaskApp.App.csproj -f net10.0-android` (0 warnings, 0 errors), and `git diff --check`. Physical/rendered validation remains required for Face Studio ergonomics, photo/camera picker UX, DIY slot overwrite, real mask image appearance, `PLAY`, `CHEC`, and `DELE`.
- [x] DIY face upload encoding correction validated with `dotnet test tests\MaskApp.Core.Tests\MaskApp.Core.Tests.csproj --no-restore --filter FullyQualifiedName~Features.Faces` (14 tests), `dotnet test tests\MaskApp.Core.Tests\MaskApp.Core.Tests.csproj --no-restore` (216 tests), `dotnet build src\MaskApp.App\MaskApp.App.csproj -f net10.0-ios --no-restore` (0 warnings, 0 errors), and `dotnet build src\MaskApp.App\MaskApp.App.csproj -f net10.0-android --no-restore` (0 warnings, 0 errors). Physical iPhone/mask validation remains required to confirm Happy Smiley renders as a recognizable yellow face on black, without the previous split colors or black patches.
