...
| --- | --- | --- | --- | --- |
| Library | [~] | Shared gallery projection for text presets, quick actions, built-in static faces, built-in animations, future/Labs placeholders, JSON-backed manual ordering, search, favorites filter, multiple grouping modes, Browse/Arrange modes, add/filter/group/manage sheets, send/manage actions, and two-column MAUI cards. | Put every reusable sendable item in one searchable, groupable, reorderable place instead of splitting it across React, RAVE, and Faces tabs. | Validate Library rendering and sends on physical iPhone/mask, then tune card density and manage flows. |
| Pages | [~] | iOS-home-screen-style shortcut pages with Use/Manage modes, page color, multiple pages, icon/color shortcut customization, add existing Library item, remove without deleting source, reorder pages/items, page editor/add-items/delete-confirmation surfaces, and shared send routing. | Let the wearer prepare live pages of small icon/text shortcuts for fast use while wearing the mask. | Validate page ergonomics and send behavior on a physical iPhone/mask. |
