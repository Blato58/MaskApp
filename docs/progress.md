...
| --- | --- | --- | --- | --- |
| Text validation/fix | [~] | Implemented | Needs real-mask test | P0 crash/freeze hotfix is implemented: async command failures are contained, Text Creator preview is debounced and list-replaced instead of cell-mutated, quick captions use centered/fitted 44-column payloads, and quick-action defaults use paced Fast write-only with Flash/Blink mode 2 at speed 100. Validate on physical iOS first, then Android. |
