...
| --- | --- | --- | --- | --- |
| Text validation/fix | [~] | Implemented | Needs real-mask test | P0 crash/freeze hotfix is implemented: async command failures are contained, Text Creator preview is debounced and list-replaced instead of cell-mutated, quick captions use centered/fitted 44-column payloads, and quick-action defaults use zero-delay Fast write-only with Flash/Blink mode 2 at speed 100. Validate on physical iOS first, then Android. |
...
| Text | [~] | Deterministic ASCII glyph rasterizer, text upload payload/framing, encrypted `DATS`/`DATCP`/`MODE`/speed commands, documented text modes, broader ACK parsing, simulator transport, MAUI composer, debounced/list-replaced LED preview, truncated diagnostics, transport state events, ACK capability reporting, ACK-required upload mode, write-only compatibility upload mode, zero-delay quick-caption send option, quick-caption 44-column centered/fitted payloads, tests, iOS upload transport, Android BLE upload transport, and compile validation. | Improve text creation/editing flow with smooth preview feedback, visible transport readiness, explicit ACK/write-only mode, send progress, no crash on text tap, no typing freeze, and actionable diagnostics before claiming real hardware parity. | Validate ACK-required and write-only text upload on physical iOS first, then Android; confirm quick captions blink/flash centered/fitted on the real mask. |
...
- [x] Physical-feedback follow-up for checklist item 8/11: quick actions now use zero-delay Fast write-only sends, and Text Creator preview refresh is debounced so typing does not synchronously rebuild the LED grid; validated with `dotnet test tests\MaskApp.Core.Tests\MaskApp.Core.Tests.csproj` (116 tests), Roslyn diagnostics (0 warnings, 0 errors), iOS build, and Android build.
