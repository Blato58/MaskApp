...
| --- | --- | --- | --- | --- |
| Text validation/fix | [~] | Implemented | Needs real-mask test | Physical feedback recorded: Fast Flash is not reliable on the real mask because it still produced left-aligned and solid text. Quick captions and Text Creator now default to the proven centered 44-column Blink path at speed 50. Background styling is disabled and fixed black after real-mask testing showed text backgrounds looked bad. Validate the updated Stable Flash defaults on physical iOS first, then Android. |
...
| Text | [~] | Deterministic ASCII glyph rasterizer, text upload payload/framing, encrypted `DATS`/`DATCP`/`MODE`/speed commands, documented text modes, broader ACK parsing, simulator transport, MAUI composer, debounced/list-replaced LED preview, truncated diagnostics, transport state events, ACK capability reporting, ACK-required upload mode, write-only compatibility upload mode, `TextSendProfile`/`TextSendPlan` package factory, Stable/Reliable quick-caption profiles with Fast Flash marked unstable, explicit Composer Scroll/Centered profiles, black-only text background behavior, serialized upload transactions, post-`DATCP` command delays, forced `SPEED` then `MODE`, stable speed/mode repeat, quick-caption 44-column centered/fitted/two-line payloads, black/off quick-caption blank columns, tests, iOS upload transport, Android BLE upload transport, and compile validation. | Improve text creation/editing flow with smooth preview feedback, visible transport readiness, explicit ACK/write-only mode, deterministic quick-caption send profiles, send progress, no crash on text tap, no typing freeze, and actionable diagnostics before claiming real hardware parity. | Revalidate the updated Stable Flash default on physical iOS: centered 44-column Blink at speed 50 with black background. Keep Fast Flash as failed/unstable until a later transport change proves it no longer left-aligns or stays solid. |
...
- [~] Port stock-protocol `MODE` and `SPEED` text controls. `BC` background styling was tried on a real mask and should stay disabled/black for now; `M`/`FC` and broader styling remain unexposed.
...
- [x] Physical text profile feedback: the checklist otherwise passed, but Fast Flash still produced left-aligned and solid text, text backgrounds looked bad, and the best observed text path was Text Creator Centered 44-column + Blink at speed 50. Defaults now follow that profile and keep the background black.
