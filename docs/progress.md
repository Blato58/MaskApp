...
- [x] Replace the live-use shell with three visible tabs: Gallery, Pages, and Control. React, RAVE, and Faces are no longer root tabs; their sendable text, quick-action, and built-in face/animation content is represented through Gallery and Pages. Text Composer and the built-in scanner/archive stay reachable as manage/add routes.
...
| --- | --- | --- | --- | --- |
| Gallery | [~] | Shared gallery projection for text presets, quick actions, built-in static faces, built-in animations, future/Labs placeholders, JSON-backed manual ordering, search, favorites filter, multiple grouping modes, send/manage actions, and two-column MAUI item rows. | Put every reusable sendable item in one searchable, groupable, reorderable place instead of splitting it across React, RAVE, and Faces tabs. | Validate Gallery rendering and sends on physical iPhone/mask, then tune card density and manage flows. |
| Pages | [~] | iOS-home-screen-style shortcut pages with page color, multiple pages, icon/color shortcut customization, add existing gallery item, remove without deleting source, reorder pages/items, and shared send routing. | Let the wearer prepare live pages of small icon/text shortcuts for fast use while wearing the mask. | Validate page ergonomics and send behavior on a physical iPhone/mask. |
...
- [x] Gallery/Pages/Control redesign validated with `dotnet test tests\MaskApp.Core.Tests\MaskApp.Core.Tests.csproj --no-restore` (190 tests), `dotnet build src\MaskApp.App\MaskApp.App.csproj -f net10.0-ios --no-restore` (0 warnings, 0 errors), `dotnet build src\MaskApp.App\MaskApp.App.csproj -f net10.0-android --no-restore` (0 warnings, 0 errors), and `git diff --check`. Physical validation remains required for rendered phone ergonomics, BLE scan/connect, auto-connect, brightness, and real mask sends.
