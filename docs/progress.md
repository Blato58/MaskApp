...
| --- | --- | --- | --- | --- |
| Text validation/fix | [~] | Implemented | Needs real-mask test | P0 deterministic text-send profiles are implemented: quick captions now use one `TextSendProfile`/`TextSendPlan` path, default to Stable Flash, build centered/fitted 44-column payloads unless scroll is explicitly selected, force `SPEED` then `MODE` after upload, and repeat speed/mode in the stable profile. Background `BC` is fail-soft and remains Needs real-mask test. Validate on physical iOS first, then Android. |
...
- [~] Port stock-protocol `MODE`, `SPEED`, and fail-soft `BC` background command planning for text profiles. `M`/`FC` and physical background behavior still need real-mask validation before broader styling is exposed.
