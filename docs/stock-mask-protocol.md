...
| `PLAY` | 1 byte count, then up to 10 DIY ids | Plays uploaded DIY image slots in order. Pages uses one slot for prepared faces/text. While MaskApp is active, app-built animations continuously repeat individual one-slot commands at their catalog-defined cadence. When the app receives its stop/lock lifecycle event, playback pauses its phone timer and makes a best-effort single-write multi-slot `PLAY` handoff so the mask can continue at its fixed, slower firmware cadence. Returning sends an immediate one-slot reclaim before configured app timing resumes. The Holy Priest set currently uses 300-400 ms per frame after real-mask testing found intermittent column artifacts at 200 ms and below. | Implemented; lock/background handoff and foreground reclaim need follow-up real-mask tests |
...
configured independently at 300-400 ms, including the requested blue → red →
...
for a short test but later failed to produce a reliable black/white flash, and
subsequent testing still showed intermittent bright or wrong-color LED columns
at 200 ms. The Holy Priest catalog therefore uses explicit per-animation delays
from 300-400 ms. iOS command playback also observes CoreBluetooth's
write-without-response availability before issuing each `PLAY`. Sustained
