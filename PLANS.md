...
| S09 - Navigation, diagnostics, recovery | Implemented | Library/Stage/Device are the stable root destinations; Stage owns Build, Preflight, Pages, Scenes, setlists, and locked performance entry. Device exposes observed profile/transport/scheduler/animation metrics, bounded reconnect and redacted-error histories, a redacted shareable report, scoped prepared-ledger reset, and guided recovery without inventing telemetry. Physical weak-link and interruption checks remain. |
| S10 - Face Studio, accessibility, ordering, localization | Implemented | Face Studio remains reachable and intact. Central dark tokens/shared controls, color-independent status, 44-point ordinary targets, festival-sized Stage controls, semantic descriptions, reduced-motion preview behavior, accessible reorder alternatives, and English/Czech resource entry points cover the redesigned primary surfaces. Broader secondary-screen string extraction and physical Dynamic Type, VoiceOver, contrast, and safe-area checks remain. |
...
|---|---|---|
| Core protocol/parsing/scheduler/profile/preflight/animation/Stage/Studio/Scene/MaskPack test suite | Verified | 441 tests passed after merging current `master` into the primary-surface redesign, including editor tools/history, Library filtering/status, Holy Priest catalog/page coverage, disconnected Preflight, persistent Stage delivery, and redacted Device diagnostics. |
| iOS compile path | Verified | Local `net10.0-ios` simulator build passed with zero warnings on 2026-07-18; signed-device execution remains separate. |
| Android compile path | Verified | Local `net10.0-android` build passed with zero warnings on 2026-07-18. |
...
| CI validation before signing | Implemented | Restore, 441 tests, and both platform builds pass locally after the `master` integration; hosted `macos-26` execution remains. |
...
| Readiness/preflight report | Implemented | Stage Preflight analyzes one Page, the whole show, or the active setlist; deduplicates/allocates slots; blocks disconnection, collisions, capacity, and unresolved content; separates blockers/warnings/verified checks; surfaces observed connection/ACK/latency/safety evidence; explains recovery; and prepares eligible DIY content. Physical cadence and profile-identity rehearsal remain. |
...
| Stage Mode | Implemented | Stage exposes a locked full-screen route with giant/2x2/dense layouts, fixed emergency controls, physical previews, current/next cue state, and persistent Pending/Sent/Failed delivery. Deterministic tests cover prepared-only triggering, hold restore, hold-to-unlock, disconnect/reconnect with no replay, and distinct Stop/Blackout emergency preemption. Physical festival ergonomics remain. |
| Consolidated Studio/import workflow | Implemented | Focused tests cover compilation/deduplication, exact revisions, compact atomic persistence and corrupt fallback, hostile import limits/signatures/timing, 46x58 conversion modes, unsaved preview semantics, Draw/Fill/Select-Move operations, bounded undo/redo, catalog projection, Preflight, and prepared replay. Native GIF/video decoders compile for iOS and Android; physical Files/import fidelity remains. |
...
| Diagnostics/support bundle/recovery | Implemented | Device presents observed capability, ACK/write, scheduler, latency/cadence, dropped-frame, slot-ledger, reconnect, and redacted-error state under progressive disclosure. The share report strips known names/identifiers, GUIDs, and MAC-like addresses; reset is limited to the active app ledger and explicitly does not erase physical content. Physical failure/recovery rehearsal remains. |
| Consolidated navigation and dormant-screen decision | Implemented | Root navigation is Library/Stage/Device. Pages and editor routes remain reachable beneath those roots; dormant Home/React/Rave registrations are preserved until their useful behavior is deliberately migrated or excluded. |
| Accessibility/localization/order hardening | Implemented | Shared semantics, text-plus-icon status, contrast tokens, reduced-motion preview gating, minimum targets, accessible ordering alternatives, and English/Czech resource infrastructure are implemented on redesigned primary surfaces. Broader secondary-screen localization and physical iPhone assistive-technology checks remain. |
